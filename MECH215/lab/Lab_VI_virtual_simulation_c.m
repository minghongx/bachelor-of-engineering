...
values = {'Enter your student number:'};
values = inputdlg(values);
st_num = floor(str2double(values));

values = {'Enter the run number:'};
values = inputdlg(values);
run_num = floor(str2double(values));

ch = cell(1,1);
ch{1} = 'Free response';
ch{2} = 'Sinsoidale (harmonic) forced response';

study = menu('Choose a case of study',ch);
...
jj2 = menu('Choose a damping',ch);

if study == 1
    values = {'Choose an initial displacement (in meter, between -0.1 and 0.1):'};
    values = inputdlg(values);
    x_init = str2double(values);
 %   x_init = input('Choose an initial displacement (enter negative value in meter): \n');
    ome_0 = 15;
else
    values = {'Choose an input frequency (rad/s): '};
    values = inputdlg(values);
    ome_0 = str2double(values);
    x_init = 0;
end
...
values = {'Choose the simulation duration (in seconds bewteen 5s and 20s): '};
values = inputdlg(values);
t_end = str2double(values);

ch = cell(1,1);
ch{1} = 'Yes';
ch{2} = 'No';
anim = menu('Do you want a graphical animation?',ch);
